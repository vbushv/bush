...
import service.ServiceManagerService;
...
private Logger logger = Logger.getLogger(getClass());
	...
	public String getList(
			...
			@RequestParam(value="searchBusinessCompany", required=false) String searchBusinessCompany,
			@RequestParam(value="searchStatus", required=false) String searchStatus,
			ModelMap modelMap,
			ProjectManagerVO projectVO,
			ServiceManagerVO serviceVO,
		...
		List<ServiceManagerVO> serviceList = null;
		...
		// GET LIST COUNT
		int totalCount = serviceManagerService.getServiceTotalCount(map);
		...
		try {
			...
			map.put("sm_name", searchServiceName);
			map.put("sm_company", searchBusinessCompany);
			map.put("sm_status", searchStatus);
			/*if(searchStatus.isEmpty() == true) {
				map.put("sm_satus", searchStatus);
			} else if(searchStatus.equals("진행") == true) {
				map.put("sm_satus", "F");
			} else if(searchStatus.equals("완료") == true) {
				map.put("sm_satus", "T");
			}*/
			
			// LOG PRINT
			logger.info("searchServiceName==>"+searchServiceName);
			logger.info("searchBusinessCompany==>"+searchBusinessCompany);
			logger.info("searchStatus==>"+searchStatus);
			
			// CUSTOMER LIST SERVICE EXECUTE
			serviceList = serviceManagerService.getServiceList(map);
		} catch(Exception e) {
		...
		try {
			...
			modelMap.put("searchServiceName", searchServiceName);
			modelMap.put("searchBusinessCompany", searchBusinessCompany);
			modelMap.put("searchStatus", searchStatus);
			/*if(serviceList == null) {
				modelMap.put("searchStatus", searchStatus);
			} else if(serviceList.get(13).equals("F") == true) {
				modelMap.put("searchStatus", "장애");
			} else if(serviceList.get(13).equals("I") == true) {
				modelMap.put("searchStatus", "기능개선");
			} else if(serviceList.get(13).equals("Q") == true) {
				modelMap.put("searchStatus", "단순문의");
			}*/
		...
		return "/serviceManager/serviceList";
	}
